# Ollie's ninth carousel. Every card must render, the arrows must work,
# the links must go somewhere real, and nothing may claim a thing that is not live.
# Punch LOOK MAY be claimed: Warm default, Soft is not the old island,
# Night with stars, a labeled chip. Synth suite and bigger dock may be
# named as already live. This update is only the punch.
import re, sys
from playwright.sync_api import sync_playwright
BASE = sys.argv[1] if len(sys.argv) > 1 else 'http://localhost:8099'
URL = BASE.rstrip('/') + '/ollie-update-9.html'
n = 0; fails = []
def ck(name, ok, got=''):
    global n; n += 1
...
def run(pw, w, hgt, label):
    ...
    pg.screenshot(path='/tmp/car9-%s.png' % label.replace(' ', '-'))
    try:
        pg.screenshot(path='/opt/cursor/artifacts/screenshots/car9-%s.png' % label.replace(' ', '-'))
    except OSError:
        pass
    b.close()
...
raw = open('ollie-update-9.html', encoding='utf-8').read()
...
ck('it names LOOK', 'LOOK' in U)
ck('it names tap', 'TAP' in U)
ck('it names hold', 'HOLD' in U)
ck('it names WARM as the default', 'WARM' in U and 'DEFAULT' in U)
ck('it names SOFT as not the old island', 'SOFT' in U and 'NOT THE OLD ISLAND' in U)
ck('it names NIGHT', 'NIGHT' in U)
ck('it names stars', 'STAR' in U)
ck('it names CRISP', 'CRISP' in U)
ck('it says the photos are real', 'A REAL PHOTO' in U)
ck('Uncle Tabs is the speaker, not a fake Ollie line', 'UNCLE TABS' in U)
ck('synth is already live, not this brick', 'ALREADY' in U and 'SYNTH' in U)
ck('bigger dock is already live, not this brick', 'ALREADY' in U and 'BIGGER' in U and 'DOCK' in U)
ck('this update is only the punch', 'ONLY THE PUNCH' in U)
ck('PLAY and DREAM stay the heroes', 'PLAY' in U and 'DREAM' in U)
