# Ollie's seventh carousel. Every card must render, the arrows must work,
# the links must go somewhere real, and nothing may claim a thing that is not live.
# Pretty modes, day/night mood, and soft corners MAY be claimed: they shipped
# in the same PR. Overnight and build replay may be mentioned as already live.
# A synth suite and a bigger dock may not.
import re, sys
from playwright.sync_api import sync_playwright
BASE = sys.argv[1] if len(sys.argv) > 1 else 'http://localhost:8099'
URL = BASE.rstrip('/') + '/ollie-update-7.html'
n = 0; fails = []
def ck(name, ok, got=''):
    global n; n += 1
...
def run(pw, w, hgt, label):
    ...
    pg.screenshot(path='/tmp/car7-%s.png' % label.replace(' ', '-'))
    try:
        pg.screenshot(path='/opt/cursor/artifacts/screenshots/car7-%s.png' % label.replace(' ', '-'))
    except OSError:
        pass
    b.close()
...
raw = open('ollie-update-7.html', encoding='utf-8').read()
...
ck('it names the three looks', 'SOFT' in U and 'WARM' in U and 'CRISP' in U)
ck('it names day or night', 'DAY' in U and 'NIGHT' in U)
ck('it names soft corners', 'SOFT CORNER' in U or 'SOFTER CORNER' in U)
