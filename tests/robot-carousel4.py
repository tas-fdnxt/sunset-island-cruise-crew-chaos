# Ollie's fourth carousel. Every card must render, the arrows must work,
# the links must go somewhere real, and nothing may claim a thing that is not live.
# Walk soccer and the Play multi-button MAY be claimed: both shipped in the same PR.
import re, sys
from playwright.sync_api import sync_playwright
BASE = sys.argv[1] if len(sys.argv) > 1 else 'http://localhost:8099'
URL = BASE + '/ollie-update-4.html'
n = 0; fails = []
def ck(name, ok, got=''):
    global n; n += 1
...
def run(pw, w, hgt, label):
    ...
    pg.screenshot(path='/tmp/car4-%s.png' % label.replace(' ', '-'))
    b.close()
...
raw = open('ollie-update-4.html', encoding='utf-8').read()
...
ck('it names the PLAY button', 'PLAY' in U)
ck('it names tap', 'TAP' in U)
ck('it names hold', 'HOLD' in U)
ck('it names the live GAMES sheet', 'GAMES SHEET' in U or 'PICK A GAME' in U or 'THE GAMES SHEET OPENS' in U)
ck('it names soccer', 'SOCCER' in U)
ck('it names all four games', all(x in U for x in
    ['SOCCER', 'WHALE SONG', 'SEAGULL BONK', 'CARGO STACK']))
ck('it claims walk soccer because that is live', 'WALK' in U and ('3D' in U or 'WHILE YOU WALK' in U))
ck('it claims the Play multi-button because that is live', 'PLAY BUTTON' in U or 'ONE BIG PLAY' in U)
...
ck('does not invent a fifth game', 'CHESS' not in U and 'PRETTY' not in U)
