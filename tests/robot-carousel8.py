# Ollie's eighth carousel. Every card must render, the arrows must work,
# the links must go somewhere real, and nothing may claim a thing that is not live.
# The synth suite and the bigger dock MAY be claimed: they shipped in the same PR.
# Pretty modes, day/night, overnight and build replay may be mentioned as already live.
import re, sys
from playwright.sync_api import sync_playwright
BASE = sys.argv[1] if len(sys.argv) > 1 else 'http://localhost:8099'
URL = BASE.rstrip('/') + '/ollie-update-8.html'
n = 0; fails = []
def ck(name, ok, got=''):
    global n; n += 1
...
def run(pw, w, hgt, label):
    ...
    pg.screenshot(path='/tmp/car8-%s.png' % label.replace(' ', '-'))
    try:
        pg.screenshot(path='/opt/cursor/artifacts/screenshots/car8-%s.png' % label.replace(' ', '-'))
    except OSError:
        pass
    b.close()
...
raw = open('ollie-update-8.html', encoding='utf-8').read()
...
ck('it names PLAY', 'PLAY' in U)
ck('it names DREAM', 'DREAM' in U)
ck('it names the synth suite', 'SYNTH' in U)
ck('it names the bigger dock', 'BIGGER DOCK' in U)
ck('it names SOUND', 'SOUND' in U)
ck('it names place', 'PLACE' in U)
ck('it says the photos are real', 'A REAL PHOTO' in U)
ck('Uncle Tabs is the speaker, not a fake Ollie line', 'UNCLE TABS' in U)
ck('pretty is already live, not this brick', 'ALREADY' in U and 'LOOK' in U)
ck('overnight is already live, not this brick', 'ALREADY' in U and 'MOON' in U)
ck('replay is already live, not this brick', 'ALREADY' in U and 'UNDO' in U)
ck('this update is only synth and dock', 'ONLY THE SYNTH' in U and 'BIGGER DOCK' in U)
