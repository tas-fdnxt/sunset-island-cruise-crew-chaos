# Ollie's sixth carousel. Every card must render, the arrows must work,
# the links must go somewhere real, and nothing may claim a thing that is not live.
# Build replay MAY be claimed: it shipped in the same PR.
# Overnight and NPC memory may be mentioned as already live.
# Pretty modes, day/night sky juice, a synth suite, and a bigger dock may not.
import re, sys
from playwright.sync_api import sync_playwright
BASE = sys.argv[1] if len(sys.argv) > 1 else 'http://localhost:8099'
URL = BASE.rstrip('/') + '/ollie-update-6.html'
n = 0; fails = []
def ck(name, ok, got=''):
    global n; n += 1
...
def run(pw, w, hgt, label):
    ...
    pg.screenshot(path='/tmp/car6-%s.png' % label.replace(' ', '-'))
    try:
        pg.screenshot(path='/opt/cursor/artifacts/screenshots/car6-%s.png' % label.replace(' ', '-'))
    except OSError:
        pass
    b.close()
...
raw = open('ollie-update-6.html', encoding='utf-8').read()
...
ck('it names UNDO', 'UNDO' in U)
ck('it names tap', 'TAP' in U)
ck('it names hold', 'HOLD' in U)
ck('it names the watch beat', 'WATCH' in U and 'GO DOWN' in U)
ck('it names keep building', 'KEEP BUILDING' in U or 'KEEP PLAYING' in U)
